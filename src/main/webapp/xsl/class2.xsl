...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="navigationlist">
		...
		<xsl:attribute name="id"><xsl:value-of select="id"/></xsl:attribute>
			...
			<ul>
				<xsl:apply-templates select="listitem" mode="verticallist"/>
			</ul>
		</div>
	</xsl:template>
	...
	<xsl:template match="list">
		<xsl:apply-templates select="header" mode="h4"/>
		<xsl:apply-templates select="listitem" mode="two_column"/>
	</xsl:template>
	...
	<xsl:template match="listitem" mode="verticallist">
		<li class="navigation">
			<a>
...
<metaInformation>
	<scenarios>
		<scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="..\xml\car_splash.xml" htmlbaseurl="" outputurl="" processortype="saxon8" useresolver="no" profilemode="0" profiledepth="" profilelength="" urlprofilexml=""
